...
{
	written=FALSE;
	update_chars(arreglito,T);
//	arreglito [2]=C_7;
	write_tablero(arreglito);
...
void read_n_save(char turn)
{
	...
	if (Get_Key_Status())
	{
		...
		input2=conv_to_format(input2);
		if(((input2&p1)==0)&&((input2&rpi)==0))		//Validacion de no ocupar un casillero ya ocupado
		{
			p1=input2|p1;				//Si se intenta llenar un casillero ya ocupado, pide nuevo valor
			casillero=0;
		}
		else
			printf("\a");
